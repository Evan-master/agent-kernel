...
type TestKernel = AgentKernel<1, 1, 1, 24, 0, 0, 0, 1, 1, 0>;

#[test]
fn facade_forwards_task_capacity_lookup_and_compaction() {
    ...
    assert_eq!(kernel.task_capacity(), 1);
    assert_eq!(kernel.task(task).unwrap().id, task);
    let receipt = kernel
        .sys_compact_task_prefix(supervisor, authority, task)
        .unwrap();
    assert_eq!(receipt.first(), task);
    assert_eq!(receipt.through(), task);
    assert_eq!(receipt.count(), 1);
    assert!(kernel.tasks().is_empty());
    assert_eq!(kernel.task(task), Err(KernelError::TaskNotFound));
}
