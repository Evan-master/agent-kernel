...
use agent_kernel_x86_64::{
    agent_call::{
        ...
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_RETIRE_AGENT_RECORD,
...
const RETIRED_FLOOR: AgentId = AgentId::new(9);

#[test]
fn agent_record_retirement_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_RETIRE_AGENT_RECORD, 36);
    ...
    assert_eq!(request.operation(), AgentCallOperation::RetireAgentRecord);
    assert!(context().authenticates(request, NONCE));
    assert!(!context().authenticates(request, NONCE + 1));
}

#[test]
fn agent_record_retirement_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn agent_record_retirement_reply_is_canonical() {
    let mut frame = request_frame();
    ...
    for (target, resource, floor) in [
        (AgentId::new(0), MANAGEMENT_RESOURCE, RETIRED_FLOOR),
        (TARGET, ResourceId::new(0), RETIRED_FLOOR),
        (TARGET, MANAGEMENT_RESOURCE, AgentId::new(0)),
    ] {
        assert_eq!(
            context()
                .encode_agent_record_retirement_reply(&mut frame, NONCE, target, resource, floor,),
...
fn request_frame() -> PrivilegeInterruptStackFrame {
    PrivilegeInterruptStackFrame {
        ...
        rsi: 8,
        rdx: 0,
        rcx: AGENT_CALL_RETIRE_AGENT_RECORD,
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_RETIRE_AGENT_RECORD);
