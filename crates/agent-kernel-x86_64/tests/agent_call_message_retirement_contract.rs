use agent_kernel_core::{AgentId, AgentImageId, CapabilityId, MessageId, TaskId};
use agent_kernel_x86_64::{
    agent_call::{
        AgentCallContext, AgentCallDecodeError, AgentCallOperation, AgentCallRequest,
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_RETIRE_MESSAGE,
        AGENT_CALL_STATUS_OK,
    },
    context::PrivilegeInterruptStackFrame,
};

const NONCE: u64 = 0xa66d_0006;
const MESSAGE: MessageId = MessageId::new(6);

#[test]
fn message_retirement_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_RETIRE_MESSAGE, 34);
    ...
    assert_eq!(request.operation(), AgentCallOperation::RetireMessage);
    assert!(context().authenticates(request, NONCE));
    assert!(!context().authenticates(request, NONCE + 1));
}

#[test]
fn message_retirement_rejects_zero_and_reserved_payloads() {
    let mut zero = request_frame();
    zero.r10 = 0;
    assert_decode_error(zero, AgentCallDecodeError::InvalidPayload);
...
#[test]
fn message_retirement_reply_returns_only_the_retired_message() {
    let mut frame = request_frame();
    let control = control_words(&frame);
    context()
        .encode_message_retirement_reply(&mut frame, NONCE, MESSAGE)
        .expect("reply encodes");

    assert_common_reply(&frame);
    assert_eq!(payload(&frame), [MESSAGE.raw(), 0, 0, 0, 0, 0, 0]);
    assert_eq!(control_words(&frame), control);
    assert_eq!(
        context().encode_message_retirement_reply(&mut frame, NONCE, MessageId::new(0)),
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_RETIRE_MESSAGE);
    assert_eq!(
