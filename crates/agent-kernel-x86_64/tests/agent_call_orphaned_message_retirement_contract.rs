use agent_kernel_core::{AgentId, AgentImageId, CapabilityId, MessageId, ResourceId, TaskId};
use agent_kernel_x86_64::{
    agent_call::{
        AgentCallContext, AgentCallDecodeError, AgentCallOperation, AgentCallRequest,
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_RETIRE_ORPHANED_MESSAGE,
        AGENT_CALL_STATUS_OK,
    },
    context::PrivilegeInterruptStackFrame,
};

const NONCE: u64 = 0xa66d_0007;
const AUTHORITY: CapabilityId = CapabilityId::new(12);
const MESSAGE: MessageId = MessageId::new(3);
const RECIPIENT: AgentId = AgentId::new(9);
const MANAGEMENT_RESOURCE: ResourceId = ResourceId::new(1);

#[test]
fn orphaned_message_retirement_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_RETIRE_ORPHANED_MESSAGE, 35);
...
#[test]
fn orphaned_message_retirement_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn orphaned_message_retirement_reply_is_canonical() {
    let mut frame = request_frame();
    ...
    for (message, recipient, resource) in [
        (MessageId::new(0), RECIPIENT, MANAGEMENT_RESOURCE),
        (MESSAGE, AgentId::new(0), MANAGEMENT_RESOURCE),
        (MESSAGE, RECIPIENT, ResourceId::new(0)),
    ] {
        assert_eq!(
            context().encode_orphaned_message_retirement_reply(
                &mut frame, NONCE, message, recipient, resource,
            ),
...
fn request_frame() -> PrivilegeInterruptStackFrame {
    PrivilegeInterruptStackFrame {
        ...
        rsi: 1,
        rdx: 0,
        rcx: AGENT_CALL_RETIRE_ORPHANED_MESSAGE,
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_RETIRE_ORPHANED_MESSAGE);
    assert_eq!([frame.rsi, frame.rdi, frame.r8, frame.r9], [1, 2, 3, NONCE]);
}
