use agent_kernel_core::{AgentId, AgentImageId, CapabilityId, FaultId, TaskId};
use agent_kernel_x86_64::{
    agent_call::{
        AgentCallContext, AgentCallDecodeError, AgentCallOperation, AgentCallRequest,
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_COMPACT_FAULTS,
        AGENT_CALL_STATUS_OK,
    },
    context::PrivilegeInterruptStackFrame,
};

const NONCE: u64 = 0xa88c_e009;
const AUTHORITY: CapabilityId = CapabilityId::new(23);
const THROUGH: FaultId = FaultId::new(4);

#[test]
fn fault_compaction_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_COMPACT_FAULTS, 39);
    ...
    assert_eq!(request.operation(), AgentCallOperation::CompactFaults);
    assert!(context().authenticates(request, NONCE));
    assert!(!context().authenticates(request, NONCE + 1));
}

#[test]
fn fault_compaction_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn fault_compaction_reply_is_canonical() {
    let mut frame = request_frame();
    let control = control_words(&frame);
    context()
        .encode_fault_compaction_reply(&mut frame, NONCE, FaultId::new(1), THROUGH, 4)
        .expect("reply encodes");

    assert_common_reply(&frame);
    assert_eq!(payload(&frame), [1, 4, 4, 0, 0, 0, 0]);
    assert_eq!(control_words(&frame), control);

    for (first, through, count) in [
        (FaultId::new(0), THROUGH, 4),
        (FaultId::new(5), THROUGH, 4),
        (FaultId::new(1), FaultId::new(0), 4),
        (FaultId::new(1), THROUGH, 0),
    ] {
        assert_eq!(
            context().encode_fault_compaction_reply(&mut frame, NONCE, first, through, count,),
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_COMPACT_FAULTS);
    assert_eq!(
