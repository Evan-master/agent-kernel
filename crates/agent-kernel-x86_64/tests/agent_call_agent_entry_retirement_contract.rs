...
use agent_kernel_x86_64::{
    agent_call::{
        ...
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_RETIRE_AGENT_ENTRY,
        AGENT_CALL_STATUS_OK,
    },
    context::PrivilegeInterruptStackFrame,
};

const NONCE: u64 = 0xa66d_0005;
const AUTHORITY: CapabilityId = CapabilityId::new(23);
const TARGET: AgentId = AgentId::new(10);

#[test]
fn agent_entry_retirement_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_RETIRE_AGENT_ENTRY, 33);
    ...
    assert_eq!(request.operation(), AgentCallOperation::RetireAgentEntry);
    assert!(context().authenticates(request, NONCE));
    assert!(!context().authenticates(request, NONCE + 1));
}

#[test]
fn agent_entry_retirement_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn agent_entry_retirement_reply_returns_only_the_retired_agent() {
    let mut frame = request_frame();
    let control = control_words(&frame);
    context()
        .encode_agent_entry_retirement_reply(&mut frame, NONCE, TARGET)
    ...
    assert_eq!(
        context().encode_agent_entry_retirement_reply(&mut frame, NONCE, AgentId::new(0)),
...
fn request_frame() -> PrivilegeInterruptStackFrame {
    PrivilegeInterruptStackFrame {
        ...
        rsi: 12,
        rdx: 0,
        rcx: AGENT_CALL_RETIRE_AGENT_ENTRY,
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_RETIRE_AGENT_ENTRY);
    assert_eq!(
