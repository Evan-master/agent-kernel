use agent_kernel_x86_64::user_memory::{
    ...
    AGENT_CODE_PAGE_CAPACITY, AGENT_RESTART_GENERATION_OFFSET, AGENT_RODATA_PAGE_CAPACITY,
...
#[test]
fn boot_event_sequence_uses_an_aligned_read_only_signal_field() {
    assert_eq!(AGENT_BOOT_EVENT_SEQUENCE_OFFSET, 8);
    assert_eq!(
        AGENT_BOOT_EVENT_SEQUENCE_OFFSET % core::mem::size_of::<u64>(),
        0
    );
    assert!(AGENT_BOOT_EVENT_SEQUENCE_OFFSET + core::mem::size_of::<u64>() <= PAGE_BYTES as usize);
    assert_ne!(AGENT_BOOT_EVENT_SEQUENCE_OFFSET, AGENT_CALL_RELEASE_OFFSET);
    assert_ne!(
        AGENT_BOOT_EVENT_SEQUENCE_OFFSET,
        PHYSICAL_QUANTUM_GENERATION_OFFSET
    );
    assert_ne!(
        AGENT_BOOT_EVENT_SEQUENCE_OFFSET,
