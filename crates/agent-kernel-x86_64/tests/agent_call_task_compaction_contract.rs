...
use agent_kernel_x86_64::{
    agent_call::{
        ...
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_COMPACT_TASKS,
        AGENT_CALL_STATUS_OK,
    },
    context::PrivilegeInterruptStackFrame,
};

const NONCE: u64 = 0xa66d_0002;
const AUTHORITY: CapabilityId = CapabilityId::new(23);
const THROUGH: TaskId = TaskId::new(6);

#[test]
fn task_compaction_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_COMPACT_TASKS, 30);
    ...
    assert_eq!(request.operation(), AgentCallOperation::CompactTasks);
    assert!(context().authenticates(request, NONCE));
    assert!(!context().authenticates(request, NONCE + 1));
}

#[test]
fn task_compaction_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn task_compaction_reply_returns_the_bounded_receipt() {
    let mut frame = request_frame();
    context()
        .encode_task_compaction_reply(&mut frame, NONCE, TaskId::new(1), THROUGH, 6)
        .expect("reply encodes");

    assert_common_reply(&frame);
    assert_eq!(payload(&frame), [1, 6, 6, 0, 0, 0, 0]);
    assert_eq!(
        context().encode_task_compaction_reply(&mut frame, NONCE, TaskId::new(0), THROUGH, 6,),
        Err(AgentCallDecodeError::InvalidPayload)
    );
    assert_eq!(
        context().encode_task_compaction_reply(&mut frame, NONCE, TaskId::new(1), THROUGH, 0,),
        Err(AgentCallDecodeError::InvalidPayload)
    );
}
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_COMPACT_TASKS);
    assert_eq!(
