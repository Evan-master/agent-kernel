use agent_kernel_core::{AgentId, AgentImageId, CapabilityId, IntentId, TaskId};
use agent_kernel_x86_64::{
    agent_call::{
        AgentCallContext, AgentCallDecodeError, AgentCallOperation, AgentCallRequest,
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_COMPACT_INTENTS,
        AGENT_CALL_STATUS_OK,
    },
    context::PrivilegeInterruptStackFrame,
};

const NONCE: u64 = 0xa66d_0003;
const AUTHORITY: CapabilityId = CapabilityId::new(23);
const THROUGH: IntentId = IntentId::new(6);

#[test]
fn intent_compaction_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_COMPACT_INTENTS, 31);
    ...
    assert_eq!(request.operation(), AgentCallOperation::CompactIntents);
    assert!(context().authenticates(request, NONCE));
    assert!(!context().authenticates(request, NONCE + 1));
}

#[test]
fn intent_compaction_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn intent_compaction_reply_returns_the_bounded_receipt() {
    let mut frame = request_frame();
    context()
        .encode_intent_compaction_reply(&mut frame, NONCE, IntentId::new(1), THROUGH, 6)
        .expect("reply encodes");

    assert_common_reply(&frame);
    assert_eq!(payload(&frame), [1, 6, 6, 0, 0, 0, 0]);
    assert_eq!(
        context().encode_intent_compaction_reply(&mut frame, NONCE, IntentId::new(0), THROUGH, 6,),
        Err(AgentCallDecodeError::InvalidPayload)
    );
    assert_eq!(
        context().encode_intent_compaction_reply(&mut frame, NONCE, IntentId::new(1), THROUGH, 0,),
        Err(AgentCallDecodeError::InvalidPayload)
    );
}
...
fn request_frame() -> PrivilegeInterruptStackFrame {
    PrivilegeInterruptStackFrame {
        ...
        rsi: 12,
        rdx: 0,
        rcx: AGENT_CALL_COMPACT_INTENTS,
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_COMPACT_INTENTS);
    assert_eq!(
