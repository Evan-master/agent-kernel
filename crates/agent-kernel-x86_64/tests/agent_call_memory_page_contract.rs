...
use agent_kernel_x86_64::{
    agent_call::{
        ...
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_ALLOCATE_MEMORY_PAGE,
        AGENT_CALL_INSPECT_MEMORY_PAGE, AGENT_CALL_MEMORY_PAGE_BYTES,
        AGENT_CALL_RELEASE_MEMORY_PAGE, AGENT_CALL_STATUS_OK,
    },
...
const VIRTUAL_BASE: u64 = 0x0000_4000_0001_7000;
const PROOF_VALUE: u64 = 0x4d45_4d50_4147_4531;

#[test]
fn memory_page_requests_decode_and_authenticate() {
    assert_eq!(
        [
            AGENT_CALL_ALLOCATE_MEMORY_PAGE,
            AGENT_CALL_INSPECT_MEMORY_PAGE,
            AGENT_CALL_RELEASE_MEMORY_PAGE,
        ],
        [21, 22, 23]
    );
    ...
    let allocate = AgentCallRequest::decode(&request_frame(
        AGENT_CALL_ALLOCATE_MEMORY_PAGE,
        [CAPABILITY.raw(), RESOURCE.raw(), 0, 0, 0, 0, 0],
    ))
    .unwrap();
    assert_eq!(
        allocate,
    ...
    assert_eq!(allocate.operation(), AgentCallOperation::AllocateMemoryPage);
...
#[test]
fn memory_page_requests_reject_zero_and_reserved_payloads() {
    for operation in [
        AGENT_CALL_ALLOCATE_MEMORY_PAGE,
        AGENT_CALL_INSPECT_MEMORY_PAGE,
        AGENT_CALL_RELEASE_MEMORY_PAGE,
        ...
        let mut reserved =
            request_frame(operation, [CAPABILITY.raw(), RESOURCE.raw(), 0, 0, 0, 0, 0]);
        reserved.r12 = 1;
...
#[test]
fn memory_page_replies_are_canonical() {
    assert_eq!(AGENT_CALL_MEMORY_PAGE_BYTES, 4096);
    ...
    context()
        .encode_memory_page_allocated_reply(&mut allocated, NONCE, CELL, VIRTUAL_BASE, 1)
        .unwrap();
    assert_common_reply(&allocated, AGENT_CALL_ALLOCATE_MEMORY_PAGE);
    assert_eq!(payload(&allocated), [1, VIRTUAL_BASE, 4096, 1, 0, 0, 0]);
    ...
    context()
        .encode_memory_page_inspected_reply(&mut inspected, NONCE, CELL, PROOF_VALUE, 1)
        .unwrap();
    assert_common_reply(&inspected, AGENT_CALL_INSPECT_MEMORY_PAGE);
    assert_eq!(payload(&inspected), [1, PROOF_VALUE, 1, 0, 0, 0, 0]);
    ...
    context()
        .encode_memory_page_released_reply(&mut released, NONCE, CELL, RESOURCE, 1)
        .unwrap();
    assert_common_reply(&released, AGENT_CALL_RELEASE_MEMORY_PAGE);
    assert_eq!(payload(&released), [1, 3, 1, 0, 0, 0, 0]);
}
