...
use agent_kernel_x86_64::{
    agent_call::{
        ...
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_COMPACT_RUNTIME_ADMISSIONS,
...
const THROUGH: RuntimeAdmissionId = RuntimeAdmissionId::new(2);

#[test]
fn runtime_admission_compaction_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_COMPACT_RUNTIME_ADMISSIONS, 29);
...
#[test]
fn runtime_admission_compaction_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn runtime_admission_compaction_reply_returns_the_bounded_receipt() {
    let mut frame = request_frame();
    context()
        .encode_runtime_admission_compaction_reply(
            &mut frame,
            NONCE,
            RuntimeAdmissionId::new(1),
            THROUGH,
            2,
        )
    ...
    assert_eq!(payload(&frame), [1, 2, 2, 0, 0, 0, 0]);
    assert_eq!(
        context().encode_runtime_admission_compaction_reply(
            &mut frame,
            NONCE,
...
fn request_frame() -> PrivilegeInterruptStackFrame {
    PrivilegeInterruptStackFrame {
        ...
        rsi: 12,
        rdx: 0,
        rcx: AGENT_CALL_COMPACT_RUNTIME_ADMISSIONS,
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_COMPACT_RUNTIME_ADMISSIONS);
    assert_eq!(
