...
fn agent_memory_identities_reject_aliases_and_prove_disjoint_frames() {
    let first =
        AgentMemoryIdentity::new(0x1000, [0x2000, 0x3000, 0x4000, 0x5000, 0x6000, 0x7000]).unwrap();
    let second =
        AgentMemoryIdentity::new(0x8000, [0x9000, 0xa000, 0xb000, 0xc000, 0xd000, 0xe000]).unwrap();
    let overlapping = AgentMemoryIdentity::new(
        0xf000,
        [0x2000, 0x10_000, 0x11_000, 0x12_000, 0x13_000, 0x14_000],
    )
    .unwrap();
    let root_overlapping = AgentMemoryIdentity::new(
        0x2000,
        [0xf000, 0x10_000, 0x11_000, 0x12_000, 0x13_000, 0x14_000],
    )
    .unwrap();
    ...
    assert!(
        AgentMemoryIdentity::new(0x1000, [0x2000, 0x2000, 0x4000, 0x5000, 0x6000, 0x7000],)
            .is_none()
    );
    assert!(
        AgentMemoryIdentity::new(0x1000, [0x1000, 0x3000, 0x4000, 0x5000, 0x6000, 0x7000])
            .is_none()
    );
    assert!(
        AgentMemoryIdentity::new(0x1001, [0x2000, 0x3000, 0x4000, 0x5000, 0x6000, 0x7000])
            .is_none()
    );
}
