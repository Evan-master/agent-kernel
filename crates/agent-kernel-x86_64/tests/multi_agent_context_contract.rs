...
fn agent_memory_identities_reject_aliases_and_prove_disjoint_frames() {
    let first = AgentMemoryIdentity::new(
        ...
        [
            0x5000, 0x6000, 0x7000, 0x8000, 0x9000, 0xa000, 0xb000, 0x2b_000,
        ],
    )
    .unwrap();
    let second = AgentMemoryIdentity::new(
        ...
        [
            0x10_000, 0x11_000, 0x12_000, 0x13_000, 0x14_000, 0x15_000, 0x16_000, 0x2c_000,
        ],
    )
    .unwrap();
    let overlapping = AgentMemoryIdentity::new(
        ...
        [
            0x5000, 0x1b_000, 0x1c_000, 0x1d_000, 0x1e_000, 0x1f_000, 0x20_000, 0x2d_000,
        ],
    )
    .unwrap();
    let table_overlapping = AgentMemoryIdentity::new(
        ...
        [
            0x24_000, 0x25_000, 0x26_000, 0x27_000, 0x28_000, 0x29_000, 0x2a_000, 0x2e_000,
        ],
    )
    .unwrap();

    assert_eq!(AGENT_CONTENT_FRAME_COUNT, 8);
    assert_eq!(AGENT_PAGE_TABLE_FRAME_COUNT, 4);
    assert_eq!(AGENT_OWNED_FRAME_COUNT, 12);
    ...
    assert!(AgentMemoryIdentity::new(
        [0x1000, 0x2000, 0x3000, 0x4000],
        [0x1000, 0x6000, 0x7000, 0x8000, 0x9000, 0xa000, 0xb000, 0x2b_000,],
    )
    .is_none());
    assert!(AgentMemoryIdentity::new(
    ...
    let zero_content = AgentMemoryIdentity::new(
        [0x1000, 0x2000, 0x3000, 0x4000],
        [0, 0x6000, 0x7000, 0x8000, 0x9000, 0xa000, 0xb000, 0x2b_000],
    )
    .unwrap();
