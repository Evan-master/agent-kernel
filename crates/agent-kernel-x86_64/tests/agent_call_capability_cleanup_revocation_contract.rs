...
use agent_kernel_x86_64::{
    agent_call::{
        ...
        AGENT_CALL_ABI_MAGIC, AGENT_CALL_ABI_VERSION, AGENT_CALL_REVOKE_CAPABILITY_FOR_CLEANUP,
        AGENT_CALL_STATUS_OK,
    },
    context::PrivilegeInterruptStackFrame,
};

const NONCE: u64 = 0xa66e_0006;
const AUTHORITY: CapabilityId = CapabilityId::new(23);
const TARGET: CapabilityId = CapabilityId::new(13);
const RESOURCE: ResourceId = ResourceId::new(3);

#[test]
fn capability_cleanup_revocation_decodes_and_authenticates() {
    assert_eq!(AGENT_CALL_REVOKE_CAPABILITY_FOR_CLEANUP, 42);
...
#[test]
fn capability_cleanup_revocation_rejects_zero_and_reserved_payloads() {
    for mutate in [
...
#[test]
fn capability_cleanup_revocation_reply_is_canonical() {
    let mut frame = request_frame();
    let control = control_words(&frame);
    context()
        .encode_capability_cleanup_revocation_reply(&mut frame, NONCE, TARGET, RESOURCE)
        .expect("reply encodes");

    assert_common_reply(&frame);
    assert_eq!(payload(&frame), [13, 3, 0, 0, 0, 0, 0]);
    assert_eq!(control_words(&frame), control);

    for (target, resource) in [
        (CapabilityId::new(0), RESOURCE),
...
fn request_frame() -> PrivilegeInterruptStackFrame {
    PrivilegeInterruptStackFrame {
        ...
        rsi: 12,
        rdx: 0,
        rcx: AGENT_CALL_REVOKE_CAPABILITY_FOR_CLEANUP,
...
fn assert_common_reply(frame: &PrivilegeInterruptStackFrame) {
    ...
    assert_eq!(frame.rdx, AGENT_CALL_REVOKE_CAPABILITY_FOR_CLEANUP);
    assert_eq!(
