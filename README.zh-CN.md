...
- 同一 ring-3 Capsule 中的原生 Agent Manager 协议：使用根作用域 `Delegate`
  ...
  身份，随后通过受认证 Agent Call 35 回收孤立 Message；
...
| 原生 ring-3 完成上下文 | 11 |
| 内核选择的 Dispatch | 35 |
| Resource Manager Agent Call | 31 |
| Resource Manager Agent/内核地址空间切换 | 62 |
...
flowchart TB
    ...
    Supervisor["ring-3 Admission Supervisor"] -->|"Agent Call 27、29-35"| X86
...
364 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行包含以下证明行：
...
- ring-3 Admission Supervisor、受认证 Agent Call 27 至 35、固定容量准入记录、
...
- 为包含 364 个事件的参考配置提供固定 2 MiB 带保护页内核启动栈。

### 后续规划

- 超出固定私有层级的动态页表增长；
- Waiter、Fault 和 Event 的有界保留与回收策略；
...
最新里程碑的完整契约见 [孤立 Message 回收设计](docs/superpowers/specs/2026-07-19-orphaned-message-retirement-v1-design.md)
和 [实现计划](docs/superpowers/plans/2026-07-19-orphaned-message-retirement-v1.md)。
