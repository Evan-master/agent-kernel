...
| `RetireMemoryCellRecord` | 43 | 在全部实时语义与架构绑定清除后回收一条终态 MemoryCell 记录 |
...
Resource 记录。
操作 43 要求调用者拥有已启动 Supervisor 入口、终态 Memory Resource 和活跃祖先
`Rollback` 权限。Core 会拒绝保留的 Namespace 引用；x86 适配层还会拒绝当前、
停驻、已完成、故障、页面、区域与共享帧池绑定。稳定稠密删除归还一个 MemoryCell
槽位，维持分配器单调增长，生成 `MemoryCellRecordRetired`，并通过固定寄存器返回完整
描述符和修订号。
