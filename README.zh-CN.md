...
| Admission Supervisor Agent Call | 24 |
| Admission Supervisor Agent/内核地址空间切换 | 48 |
...
flowchart TB
    ...
    Supervisor["ring-3 Admission Supervisor"] -->|"Agent Call 27、29、30、31、32"| X86
...
356 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行包含以下证明行：
...
- ring-3 Admission Supervisor、受认证 Agent Call 27 至 32、固定容量准入记录、
...
- 为包含 356 个事件的参考配置提供固定 2 MiB 带保护页内核启动栈。
...
最新里程碑的完整契约见 [Capability Store 回收设计](docs/superpowers/specs/2026-07-19-capability-store-compaction-v1-design.md)
和 [实现计划](docs/superpowers/plans/2026-07-19-capability-store-compaction-v1.md)。
