...
| Admission Supervisor Agent Call | 17 |
| Admission Supervisor Agent/内核地址空间切换 | 34 |
...
flowchart TB
    ...
    Supervisor["ring-3 Admission Supervisor"] -->|"Agent Call 27、29、30"| X86
...
344 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行包含以下证明行：
...
- ring-3 Admission Supervisor、受认证 Agent Call 27 至 30、固定容量准入记录、
...
- 为包含 344 个事件的参考配置提供固定 2 MiB 带保护页内核启动栈。

### 后续规划

- 超出固定私有层级的动态页表增长；
- Intent Store 压缩，以及 Capability、Agent Entry、Message、Waiter、Fault 和 Event
  的有界保留策略；
...
最新里程碑的完整契约见 [Task Store 回收设计](docs/superpowers/specs/2026-07-19-task-store-compaction-v1-design.md)
和 [实现计划](docs/superpowers/plans/2026-07-19-task-store-compaction-v1.md)。
