...
| Admission Supervisor Agent Call | 16 |
| Admission Supervisor Agent/内核地址空间切换 | 32 |
...
328 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行包含以下证明行：
...
- 为包含 328 个事件的参考配置提供固定 2 MiB 带保护页内核启动栈。
