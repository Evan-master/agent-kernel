...
- 同一 ring-3 Capsule 中的原生 Agent Manager 协议：使用根作用域 `Delegate`
  ...
  启动的身份；
- 由真实物理帧支撑的运行时内存页生命周期：创建自身拥有的 Memory Resource，
  映射一个用户态可读写且禁止执行的私有页，观察 ring-3 证明写入，随后解除映射
  并清零保留帧；
...
| 真实物理时间片到期 | 10 |
| 被隔离的 Agent 故障 | 4 |
| Resource Manager 执行后的资源 | 3 |
| Resource Manager 执行后的 Capability | 14 |
| Resource Manager 执行后的 Intent | 7 |
| Resource Manager 执行后的 Task | 7 |
| Resource Manager 执行后的 MemoryCell | 1 |
| Driver 完成后的有序内核事件 | 185 |
...
运行时内存页调用只接受 Capability 与内核对象句柄；虚拟地址、物理帧、访问标志
和页大小均由内核选择。
...
185 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行的结尾是：
...
- 裸机 x86_64 隔离、时钟抢占、故障隔离/恢复、原生资源生命周期 Agent Call
  以及原生 Capability、Intent、Task、受管 Agent 和真实物理运行时内存页
  生命周期 Agent Call。

### 后续规划

- 跨 Agent 共享的运行时帧池、多页区域和动态中间页表分配；
...
最新里程碑的完整契约见 [原生内存页设计](docs/superpowers/specs/2026-07-18-x86-native-memory-page-v0-design.md)
和 [实现计划](docs/superpowers/plans/2026-07-18-x86-native-memory-page-v0.md)。
