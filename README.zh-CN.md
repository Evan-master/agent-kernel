...
- 阻塞式邮箱发送、接收、确认、接收方所有的终态 Message 回收与唤醒，主动 Yield，
  任务结果、目标限定验证和完成；
...
- 四条受认证 Worker 完成通知，分两轮唤醒保留的 Supervisor 调用帧，并完成 FIFO
  接收、确认与回收；Message 3 至 6 在两个批次间复用两个瞬时槽位；
...
| Admission Supervisor Agent Call | 30 |
| Admission Supervisor Agent/内核地址空间切换 | 60 |
...
flowchart TB
    ...
    Supervisor["ring-3 Admission Supervisor"] -->|"Agent Call 27、29、30、31、32、33、34"| X86
...
362 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行包含以下证明行：
...
- ring-3 Admission Supervisor、受认证 Agent Call 27 至 34、固定容量准入记录、
...
- 为包含 362 个事件的参考配置提供固定 2 MiB 带保护页内核启动栈。

### 后续规划

- 超出固定私有层级的动态页表增长；
- 退休 Agent 所有 Pending Message 的管理策略，以及 Waiter、Fault 和 Event 的
  有界保留策略；
...
最新里程碑的完整契约见 [Message 回收设计](docs/superpowers/specs/2026-07-19-message-retirement-v1-design.md)
和 [实现计划](docs/superpowers/plans/2026-07-19-message-retirement-v1.md)。
