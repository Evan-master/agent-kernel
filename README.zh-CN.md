...
- 每次成功修改都会生成确定性审计证据。普通修改追加有序 **事件（Event）**；
  Event 归档提交会推进加密检查点链，使已满日志能够释放容量。

原生模型中不存在环境式的“默认超级用户”。Agent 可以拥有很高权限，但这些
权限必须由明确的 Capability 表达，并且始终能在审计链中观察到。
...
- Agent Call 38，允许通过认证的 Supervisor 使用共享 `Rollback` 清理权限压缩连续
  ...
  Waiter 生成完整审计事件；
- Agent Call 39，允许通过认证的 Supervisor 使用共享 `Rollback` 清理权限压缩连续
  已恢复 Fault 前缀，拒绝仍被活跃 Task 或 Message 引用的记录，清理安全的 Task
  历史指针，保持 Fault ID 单调增长，并为每条回收 Fault 生成完整审计事件；
- Agent Call 40，允许已启动 Supervisor 使用根作用域 `Rollback` 权限准备并提交
  稠密 Event 前缀归档，保留链式 SHA-256 检查点，维持 Event 序列单调增长，并在
  实时 Event Log 已满时恢复容量；
- Agent Call 41，允许已启动 Supervisor 使用活跃祖先 `Rollback` 权限回收一条
  终态且无引用的 Resource 记录，归还稠密 Store 槽位，维持 Resource ID 单调增长，
  并生成完整定宽回执和有序审计 Event；
- Agent Call 42，允许已启动 Supervisor 使用活跃祖先 `Rollback` 权限撤销退休
  Resource 上的残留权限，为 Resource 记录回收前的叶节点优先 Capability 清理
  建立授权路径；
- 64 条记录的架构归档交接：357 个实时 Event 槽位全部占用时捕获 Event 1 至 64，
  在 ring 3 校验检查点，从 Event 358 继续执行，最终将归档迭代器与实时迭代器合并
  为 Event 1 至 383 的精确转录；
...
- 三槽 x86 Waiter Store 容量证明：Supervisor 在第一轮通知结束后回收 Waiter 1
  ...
  Store 归零；
- 四槽 x86 Fault Store 容量证明：Fault Worker 使用 Fault 1 至 4 填满 Store，终态
  Task 前缀回收清除实时阻塞引用，常驻 Supervisor 随后通过 Agent Call 39 回收全部
  Fault 记录，使活跃 Store 归零并保留完整历史 Event；
...
| Admission Supervisor Agent Call | 38 |
| Admission Supervisor Agent/内核地址空间切换 | 76 |
...
flowchart TB
    ...
    Core --> Events["有界实时 Event Log"]
    Core --> ArchiveHead["Event 归档检查点链头"]
    ...
    Supervisor["ring-3 Admission Supervisor"] -->|"Agent Call 27、29-42"| X86
...
| `CompactFaults` | 39 | 从稠密 Fault Store 回收已授权的连续恢复态前缀 |
| `ArchiveEvents` | 40 | 提交通过认证的 Event 前缀交接并返回链式摘要 |
| `RetireResourceRecord` | 41 | 从稠密 Resource Store 回收一条无引用的终态 Resource 记录 |
| `RevokeCapabilityForCleanup` | 42 | 使用活跃祖先权限撤销退休 Resource 上的残留权限 |

原生 Resource 创建 ABI 当前接受 Workspace、Memory、Service、Network 和 Device。
Resource 回收回复也能编码 Core 中已保留的 File 与 Process 记录。未知创建类型、未知
操作位、零句柄、过期 Nonce、错误身份或非零保留寄存器都会直接拒绝。
...
操作 39 要求调用者通过活跃 Supervisor 身份认证，并对每个选中 Fault Resource
拥有共享清理权限。处于 `Faulted` 状态且引用选中记录的 Task，以及携带选中 Fault
引用的未确认 Message，都会阻止整项事务。活跃 Resource 要求精确 `Rollback`
范围，退休 Resource 接受活跃祖先范围。聚合 Event 预检成功后，事务清理安全的
Task `last_fault` 指针，按稳定顺序移除前缀，保持 Fault 分配器单调增长，并通过
`FaultCompacted` 记录原始 Agent、Task、Resource、Fault 种类、详情、身份、
执行者和授权 Capability。
操作 40 要求调用者拥有已启动 Supervisor 入口，以及活跃、无 Task 作用域、根作用域
的 `Rollback` Capability。准备阶段以只读方式选择非空连续实时前缀，并对全部 Event
字段、归档代数和前序摘要执行规范 SHA-256 计算。提交阶段先重新计算并比较完整提案，
随后按稳定顺序移除前缀。后续 Event 序列继续单调增长；专用检查点记录执行者、授权、
根 Resource、范围、数量、前序摘要和当前摘要。x86 回复返回范围、数量和四个小端
摘要字，全程不接受用户态指针；归档代数保留在 Core 检查点中。
操作 41 要求调用者拥有已启动 Supervisor 入口，目标 Resource 处于 `Retired`
状态，并持有活跃祖先 `Rollback` 权限。事务会拒绝全部保留的非 Event 引用，预留
一个 Event 槽位，按稳定稠密顺序移除目标，保持 Resource 分配器单调增长，并返回
完整记录与 `ResourceRecordRetired` 审计证据。x86 回复通过固定寄存器携带 Resource
身份、稳定种类编码、可选父 Resource 和可选所有者。
操作 42 要求调用者拥有已启动 Supervisor 入口，以及活跃、无 Task 作用域的
`Rollback` 权限；授权 Resource 必须位于目标退休 Resource 的不可变祖先链。完整
校验通过后，事务只撤销一条残留 Capability，并生成包含清理授权和 `Rollback` 操作的
`CapabilityRevoked`。随后通过既有叶节点优先压缩归还稀疏槽位，再由操作 41 移除
Resource 记录。
...
383 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行包含以下证明行：
...
- ring-3 Admission Supervisor、受认证 Agent Call 27 与 29 至 42、固定容量准入记录、
...
- 仅限 Supervisor 发起的退休 Resource 残留 Capability 撤销，通过活跃祖先
  `Rollback` 权限建立授权路径，随后执行叶节点优先稀疏清理；
- 经过授权的稠密 Resource 记录回收，执行终态与完整非 Event 引用预检、活跃祖先
  授权、单调 ID 和稳定保留顺序，生成定宽回执，完成槽位复用并保留完整 Event 证据；
...
- 经过授权的非活跃 Waiter 前缀压缩，执行聚合 Event 预检和共享 Resource 清理
  ...
  逐记录 Event 证据；
- 经过授权的恢复态 Fault 前缀压缩，执行活跃 Task 与 Message 引用预检，清理安全的
  Task 历史指针，使用共享 Resource 清理权限，保持留存顺序稳定与 Fault ID 单调
...
最新里程碑的完整契约见 [Resource 记录回收设计](docs/superpowers/specs/2026-07-19-resource-record-retirement-v1-design.md)
和 [实现计划](docs/superpowers/plans/2026-07-19-resource-record-retirement-v1.md)。
