...
Agent Kernel 是一个用 Rust 编写的实验性 Agent 原生操作系统内核。它不是
Linux 发行版，不是控制命令行的 Agent，也不是以 POSIX 兼容为中心的套壳。
它从 Agent、资源、能力、意图、任务、事件、验证和回滚这些原生内核对象出发。

> **项目状态：** 早期研究原型。目前已经能在 QEMU 中启动独立的 x86_64
> 裸机内核并运行相互隔离的 ring-3 Agent Capsule，但还不是通用或生产级操作系统。
...
当前 BIOS/QEMU 证明链不依赖 Linux 或其他宿主操作系统作为内核底座，已经实现：
...
- 真实 ring-3 Resource Manager：使用派生的 `Act` 权限创建子 Service，
  接收新 Capability，再以其中的 `Rollback` 权限退休该 Service；
...
| Resource Manager 执行后的 Capability | 11 |
| Driver 完成后的有序内核事件 | 169 |

`scripts/run-qemu.sh` 会逐条检查事件顺序，同时拒绝缺失标记、多余事件、错误的
...
内核刻意保持小型、确定性和可检查。LLM 推理、Prompt、远程模型调用和高级规划
应该放在用户态 Supervisor，而不是内核空间。
...
| `agent-kernel-x86_64` | 裸机 x86_64 启动、隔离、中断、故障、Agent Call 与 QEMU 证明 |
...
169 个事件，并把内核 debug-exit 状态也作为契约的一部分。成功运行的结尾是：
...
- Capsule、Agent Call、CPU 帧、事件序列或物理所有权证据不匹配时，系统直接
  终止 QEMU 证明，不会降级到宽松模式。

这个项目探索的是高权限 Agent。目标不是把 Agent 限制成普通应用权限，而是让
广泛权限变得明确、可组合、可撤销、可审计。
...
- Capability 授予与收窄、Task 委派、来源撤销传播、资源所有权和资源退休；
- 固定容量调度、等待/唤醒、邮箱 IPC、故障策略、镜像验证、运行时准入和 Driver
  Invocation 生命周期；
- 裸机 x86_64 隔离、时钟抢占、故障隔离/恢复和原生资源生命周期 Agent Call。
...
最新里程碑的完整契约见 [Resource Manager 设计](docs/superpowers/specs/2026-07-17-x86-native-resource-manager-agent-v0-design.md)
和 [实现计划](docs/superpowers/plans/2026-07-17-x86-native-resource-manager-agent-v0.md)。
历史设计记录保留在 `docs/superpowers/specs/`。

## 参与贡献

这是一个公开仓库。任何人都可以 Fork 并提交 Pull Request，但公开可见不等于
拥有 Push 或 Merge 权限；最终由仓库维护者决定哪些代码进入 `main`。

修改代码前请阅读 [AGENTS.md](AGENTS.md)，尤其注意：

- 保持 Agent 原生模型，不要让 POSIX 抽象反过来定义核心；
- 保持 `no_std`、确定性、固定容量存储和明确事件；
- 新运行时行为必须先写失败测试；
- 不得增加 Capability 绕过或隐藏的特权修改路径；
- 发布前运行工作区测试、Supervisor 和相关 QEMU 证明。
