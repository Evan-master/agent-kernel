...
if [[ "$PROFILE" == "debug" ]]; then
  IMAGE="$ROOT_DIR/target/agent-kernel-x86_64-dma-iommu-bios.img"
else
  IMAGE="$ROOT_DIR/target/agent-kernel-x86_64-dma-iommu-release-bios.img"
fi
...
if [[ "$PROFILE" == "release" ]]; then
  rustup run "$TOOLCHAIN" cargo build \
    ...
    --features bare-metal,qemu-dma-iommu-proof \
    --target "$TARGET" \
    --release
else
  rustup run "$TOOLCHAIN" cargo build \
