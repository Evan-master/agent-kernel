...
if [[ "$PROFILE" == "debug" ]]; then
  IMAGE="$ROOT_DIR/target/agent-kernel-x86_64-msi-msix-bios.img"
else
  IMAGE="$ROOT_DIR/target/agent-kernel-x86_64-msi-msix-release-bios.img"
fi
...
if [[ "$PROFILE" == "release" ]]; then
  rustup run "$TOOLCHAIN" cargo build \
    ...
    --features bare-metal,qemu-msi-msix-proof \
    --target "$TARGET" \
    --release
else
  rustup run "$TOOLCHAIN" cargo build \
