...
if [[ "$PROFILE" == "debug" ]]; then
  IMAGE="$ROOT_DIR/target/agent-kernel-x86_64-native-udp-driver-bios.img"
else
  IMAGE="$ROOT_DIR/target/agent-kernel-x86_64-native-udp-driver-release-bios.img"
fi
...
if [[ "$PROFILE" == "release" ]]; then
  rustup run "$TOOLCHAIN" cargo build \
    ...
    --features bare-metal,qemu-native-udp-driver-proof \
    --target "$TARGET" \
    --release
else
  rustup run "$TOOLCHAIN" cargo build \
