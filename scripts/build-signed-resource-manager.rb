...
# Builds the canonical signed Resource Manager Agent Package from assembly.
# The Ed25519 private key is supplied externally and is never copied into the
# repository or emitted in command output.
...
SOURCE = File.join(ROOT, "crates/agent-kernel-x86_64/src/boot_agent_images/resource_manager.S")
MAGIC = "AGNTIMG\0".b
HEADER_BYTES = 88
SEGMENT_BYTES = 24
RELOCATION_BYTES = 24
...
def fail_with(message)
  warn "build signed Resource Manager failed: #{message}"
  exit 1
end
...
OptionParser.new do |parser|
  parser.banner = "usage: scripts/build-signed-resource-manager.rb --key PATH --output PATH"
...
package = nil
Dir.mktmpdir("agent-kernel-resource-manager") do |directory|
  object = File.join(directory, "resource_manager.o")
  code_path = File.join(directory, "resource_manager.text.bin")
  rodata_path = File.join(directory, "resource_manager.rodata.bin")
  ...
  fail_with("ABS64 relocation placeholder is nonzero") unless code.byteslice(2, 8) == "\0" * 8

  relocation_offset = HEADER_BYTES + 2 * SEGMENT_BYTES
  code_offset = relocation_offset + RELOCATION_BYTES
  ...
  header = MAGIC + [3, 1, 4, 1, 1, 1, 0, 0].pack("v8")
  header << [0].pack("V")
  header << [2, 1].pack("v2")
